...
def get_projects():
    ...
    level = None if "block_detail" in request.args and request.args["block_detail"] == "all" else 2
    ...
    # Order columns
    cols = ["project_id", "title", "author", "description", "instructions", "studio_id"]
    stats = list()
...
def get_studios():
    ...
    try:
        if int(request.args["id"]) > 0:
            studios = json.loads(scrape.Studio.objects(studio_id=request.args["id"]).exclude("id").to_json())
    except:
    ...
    # Order columns
    cols = ["studio_id", "title", "description", "status", "challenge_id.$oid", "public_show"]
    stats = list()
    for row in flattened:
        if row not in stats and row not in cols:
            stats.append(row)
