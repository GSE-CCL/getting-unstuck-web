...
def test_add_schema_correct(credentials):
    result = schema.add_schema(credentials_file=credentials,
                               mins={"instructions_length": 50, "description_length": 50, "comments_made": 1},
                               min_blockify={"comments": 3, "costumes": 1, "sounds": 0, "sprites": 1},
...
def test_add_schema_incorrect_rc(credentials):
    result = schema.add_schema(credentials_file=credentials, required_block_categories=[])
    assert result == False

def test_schema_object_incorrect_rb(credentials):
    result = schema.add_schema(credentials_file=credentials, required_blocks={"name": "event_whenflagclicked"})
    assert result == False
