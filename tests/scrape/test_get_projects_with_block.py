...
def test_get_projects_with_block_none(tmp_path, credentials):
    scrape.add_studio(26211962,
                      cache_directory=tmp_path,
                      credentials_file=credentials)
    result = scrape.get_projects_with_block("nonexistent_block",
                                            credentials_file=credentials)
...
def test_get_projects_with_block(tmp_path, credentials):
    ...
    ct = 0
    while ct < 4:
        # Sometimes the objects are None
        try:
            ct = len(scrape.Project.objects(studio_id=26211962))
        except:
            pass

        continue

    result = scrape.get_projects_with_block("control_repeat",
                                            credentials_file=credentials)
